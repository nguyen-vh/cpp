...
class SFML_AUDIO_API SoundStream : public SoundSource {
 ...
 private:
  ...
  ///
  ////////////////////////////////////////////////////////////
  [[nodiscard]] bool fillAndPushBuffer(unsigned int bufferNum,
  ...
  static constexpr unsigned int BufferCount{
      3};  //!< Number of audio buffers used by the streaming loop
  static constexpr unsigned int BufferRetries{
  ...
  Status m_threadStartState{
      Stopped};  //!< State the thread starts in (Playing, Paused, Stopped)
  ...
  std::uint64_t m_samplesProcessed{};  //!< Number of samples processed since
                                       //!< beginning of the stream
  std::int64_t
      m_bufferSeeks[BufferCount]{};  //!< If buffer is an "end buffer", holds
                                     //!< next seek position, else NoLoop. For
                                     //!< play offset calculation.
  Time m_processingInterval{milliseconds(
      10)};  //!< Interval for checking and filling the internal sound buffers.
};
