...
#define C_CLASS_HANDLING() template <typename T> bool check_class( ) { if ( !std::is_same<T , TASK::TESTER::No>::value ) {  std::string strClassN = typeid( T ).name( ) , strClassNreadable {}; strClassN = strClassN.erase( 0 , 9 ); for ( int i = 0; i < strClassN.length( ); ++i ) { if ( std::isdigit( strClassN [ i ] ) ) {  if ( !std::isdigit( strClassN [ i + 1 ] ) ) { strClassNreadable = strClassN.substr( i + 1 ); } } } strClassNreadable.pop_back( ); std::cout << "+ " << strClassNreadable << " found" << std::endl; return true; } else { std::cout << "- Class in Assignment not found. " << std::endl;  std::cout << "-  > Members not found" << std::endl; return false; }} 
#define C_MAIN( ExpectedOutput , check_Classname1 , check_Classname2 , check_Classname3 , check_Classname4 , check_Classname5 ) void evaluation( const std::string& expectedOutput ,const bool& Bt1 , const bool& Bt2 , const bool& Bt3 , const bool& Bt4 , const bool& Bt5 ) { if ( Bt1 && Bt2 && Bt3 && Bt4 && Bt5 ) { if ( check_output( *( STUDENT::main ) , expectedOutput ) ) { std::cout << std::endl; std::cout << "+ + Student did great Job! + +\n" << std::endl; }} else { std::cout << std::endl; std::cout << "- Student forgot a Member inside a Class!\n" << std::endl; }} namespace STUDENT::TASK { using namespace ::TASK::TESTER; void testing( ) { bool Bt1 = check_Classname1; bool Bt2 = check_Classname2; bool Bt3 = check_Classname3; bool Bt4 = check_Classname4; bool Bt5 = check_Classname5; std::string expectedOutput = ExpectedOutput; ::evaluation( expectedOutput , Bt1 , Bt2 , Bt3 , Bt4 , Bt5 ); }} int main( ) { STUDENT::TASK::testing( ); return 0; }
namespace STUDENT {
